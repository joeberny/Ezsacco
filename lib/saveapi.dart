...
class save extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Flutter Demo',
      home: MyHomePage(title: 'Save With Ez-SacCO'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  String publicKey = "FLWPUBK_TEST-f52e9f4829f53c980df30658d789598b-X";
  String encryptionkey = "FLWSECK_TESTde89f023dab7";
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: Color.fromARGB(255, 124, 124, 25),
      ...
      body: Container(
        decoration: BoxDecoration(
          image: DecorationImage(
            image: AssetImage("assets/sacco1.jpg"),
            fit: BoxFit.cover,
          ),
        ),
  ...
  _handlePaymentInitialization() async {
    final flutterwave = Flutterwave.forUIPayment(
        ...
        encryptionKey: this.encryptionkey.trim(),
    ...
    bool checkPaymentIsSuccessful(final ChargeResponse response) {
      return response.data!.status == FlutterwaveConstants.SUCCESSFUL &&
          response.data!.currency == this.currencyController.text &&
          response.data!.amount ==
              this.amountController.text.toString().trim() &&
          response.data!.txRef == DateTime.now().toIso8601String();
    }
  }
